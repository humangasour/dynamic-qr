...
export async function generateMetadata({ params }: RedirectPageProps): Promise<Metadata> {
  ...
  const t = await getTranslations('redirect.page.slug');
  return {
    title: t('metadataTitle'),
    description: t('metadataDescription', { slug }),
  };
}
 ...
 */
async function NotFoundFallback({ slug }: { slug: string }) {
  const t = await getTranslations('redirect.page.slug');
  return (
        ...
        <div className="max-w-md w-full bg-card shadow-lg rounded-lg p-8 text-center mx-auto">
          <div className="mb-6">
            ...
            <p className="text-muted-foreground mb-4">
              {t.rich('notFoundDescription', {
                code: (chunks) => (
                  <code className="bg-muted px-2 py-1 rounded text-sm font-mono">{chunks}</code>
                ),
                slug,
              })}
            </p>
          ...
          <div className="space-y-3">
            ...
            <ul className="text-sm text-muted-foreground text-left space-y-1">
              <li>• {t('reasons.deleted')}</li>
              <li>• {t('reasons.deactivated')}</li>
              <li>• {t('reasons.typo')}</li>
            </ul>
          </div>
